...
export default function FlowWin({
  levelId,
  moves,
  pipes,
  hintsUsed,
  ...
  const perfect = moves === pipes && hintsUsed === 0;

  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label={`Nivel ${levelId} resuelto`}
    ...
    >
      ...
      <div className="animate-rise relative w-full max-w-sm rounded-3xl border-[3px] border-tinta bg-crema p-6 text-center shadow-[0_12px_0_rgba(90,42,51,0.3)]">
        ...
        <p className="mt-1 font-bold text-tinta/70">
          Uniste los {pipes} tubos y llenaste el tablero
        </p>

        <dl className="mt-4 flex gap-2">
          <Stat label="Movidas" value={`${moves}`} tone="bg-blush/50" />
          <Stat label="Tubos" value={`${pipes}`} tone="bg-menta/70" />
